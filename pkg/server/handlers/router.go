...
func NewHandler(ctx context.Context, control *config.Control, cfg *cmds.Server) http.Handler {
	...
	// authed.Handle(prefix+"/client-supervisor.crt", ClientSupervisorCert(control))
	// authed.Handle(prefix+"/client-kube-apiserver.crt", ClientKubeAPIServerCert(control))
	// authed.Handle(prefix+"/client-controller.crt", ClientControllerCert(control))
	// authed.Handle(prefix+"/client-scheduler.crt", ClientSchedulerCert(control))
	// authed.Handle(prefix+"/client-admin.crt", ClientAdminCert(control))
	// authed.Handle(prefix+"/client-{program}-cloud-controller.crt", ClientProgramCloudControllerCert(control))
