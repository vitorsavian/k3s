...
func NewHandler(ctx context.Context, control *config.Control, cfg *cmds.Server) http.Handler {
	...
	authed.Handle(prefix+"/client-{program}-controller.crt", ClientProgramControllerCert(control))
	...
	// server specific certs
	serverAuthed.Handle(prefix+"/client-supervisor.crt", ClientSupervisorCert(control))
	serverAuthed.Handle(prefix+"/client-kube-apiserver.crt", ClientKubeAPIServerCert(control))
	serverAuthed.Handle(prefix+"/client-controller.crt", ClientControllerCert(control))
	serverAuthed.Handle(prefix+"/client-scheduler.crt", ClientSchedulerCert(control))
	serverAuthed.Handle(prefix+"/client-admin.crt", ClientAdminCert(control))
	serverAuthed.Handle(prefix+"/client-{program}-cloud-controller.crt", ClientProgramCloudControllerCert(control))
	serverAuthed.Handle(prefix+"/peer-server-client.crt", EtcdPeerCert(control))
	serverAuthed.Handle(prefix+"/server-client.crt", EtcdServerCert(control))
	serverAuthed.Handle(prefix+"/client.crt", EtcdClientCert(control))
	serverAuthed.Handle(prefix+"/serving-kube-apiserver.crt", ServingKubeAPICert(control))
	serverAuthed.Handle(prefix+"/client-auth-proxy.crt", ClientAuthProxy(control))
	serverAuthed.Handle(prefix+"/kube-scheduler.crt", ServingKubeSchedulerCert(control))
	serverAuthed.Handle(prefix+"/kube-controller-manager.crt", ServingKubeControllerManagerCert(control))
	serverAuthed.Handle(prefix+"/dynamic-listener", DynamicListenerCert(control))
