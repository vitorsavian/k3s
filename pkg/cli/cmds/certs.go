...
var (
	...
	CertRotateReloadCommandFlags = []cli.Flag{
		DebugFlag,
		LogFile,
		AlsoLogToStderr,
		DataDirFlag,
		...
		&cli.StringFlag{
			Name:        "token",
			Aliases:     []string{"t"},
			Usage:       "Token to authenticate with the server",
			EnvVars:     []string{version.ProgramUpper + "_TOKEN"},
			Destination: &ServerConfig.Token,
		},
		&cli.StringFlag{
			Name:        "server",
			Usage:       "(cluster) Server to download certificates from",
...
func NewCertCommands(check, rotate, reload, rotateCA func(ctx *cli.Context) error) *cli.Command {
	return &cli.Command{
		Name:            CertCommand,
		...
		Subcommands: []*cli.Command{
			...
			},
			{
				...
				Subcommands: []*cli.Command{
					{
						Name:            "reload",
						Usage:           "Reload " + version.Program + " component certificates on disk while running",
						SkipFlagParsing: false,
						Action:          reload,
						Flags:           CertRotateReloadCommandFlags,
					},
				},
			},
